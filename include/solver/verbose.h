...
struct VerboseElement {
    std::pair<const Vector&, int64_t> value;
    bool is_empty;

    VerboseElement(std::pair<const Vector&, int64_t> value, bool is_empty): value(value), is_empty(is_empty) {}
};

template<typename Vector>
class VerboseUnit {
    using Variant = std::pair<Vector, int64_t>;

    std::shared_ptr<std::vector<Vector>> variants;
    std::shared_ptr<std::vector<int64_t>> scores;
...
public:
    ...
    VerboseUnit(const Variant picked_variant): variants(std::make_shared<Vector>({picked_variant.first})),
        scores(std::make_shared<std::vector<int64_t>>({picked_variant.second})), picked_id(0), empty(false),
        variants_mask(std::make_shared<std::vector<bool>>((*variants).size())) {}
    
    VerboseUnit(const Vector& solution, int64_t score): 
        variants(std::make_shared<std::vector<Vector>>(std::initializer_list<Vector>({solution}))),
        scores(std::make_shared<std::vector<int64_t>>(std::initializer_list<int64_t>({score}))), picked_id(0), empty(false),
        variants_mask(std::vector<bool>(variants->size())) {}

    VerboseUnit(std::shared_ptr<std::vector<Vector>> variants, std::shared_ptr<std::vector<int64_t>> scores,
    ...
    VerboseElement<Vector> operator[](int64_t i) const {
        return VerboseElement<Vector>(std::pair<const Vector&, int64_t>(variants->at(i), scores->at(i)), variants_mask.at(i));
    }

    template<typename Field>
    void PrintFitnessLandScape(const std::unordered_map<int64_t, int64_t>& landscape, Field& field, int64_t n) {
        std::cout << "\n============ Fitness Landscape ============\n";
        int64_t i=0;
