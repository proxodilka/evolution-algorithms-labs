...
template<typename AlphabetType, typename VectorType = typename Utils::rebind_container<AlphabetType, SerachFields::Vector<void>>::value >
std::function<int(const VectorType&)> build_random(const AlphabetType& alphabet) {
    int min = 0, max=RAND_MAX;
    auto fn = [=](const VectorType& vector) { return Utils::random(min, max); };
    return Utils::cache_calls<int>(fn, std::bind(Utils::ToInteger<VectorType, AlphabetType>, std::placeholders::_1, alphabet));
};
...
template<typename AlphabetType, typename VectorType = typename Utils::rebind_container<AlphabetType, SerachFields::Vector<void>>::value >
std::function<int(const VectorType&)> build_bin_to_int(const AlphabetType& alphabet) {
    auto fn = [=](const VectorType& vector) { return Utils::ToInteger(vector, alphabet); };
    return Utils::cache_calls<int>(fn, std::bind(Utils::ToInteger<VectorType, AlphabetType>, std::placeholders::_1, alphabet));
};

template<typename AlphabetType, typename VectorType = typename Utils::rebind_container<AlphabetType, SerachFields::Vector<void>>::value >
std::function<int(const VectorType&)> build_square(const AlphabetType& alphabet, int field_size) {
